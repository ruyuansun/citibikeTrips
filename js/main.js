...
let loadTripData = function () {
  let url = `https://tame-jade-lizard-sock.cyclic.app/api/trips?page=${page}&perPage=${perPage}`;
  fetch(url)
    .then((response) => response.json())
    .then((data) => {
      tripData = data;
      let tableRowsString = tripData
        .map((trip) => objectToTableRow(trip))
        .join("");
      ...
      document.querySelectorAll("#trips-table tbody tr").forEach((row) => {
        row.addEventListener("click", (e) => {
          currentTrip = tripData.find(
            (trip) => trip._id === row.getAttribute("data-id")
          ); //Finds the first element in an array that satisfies the condition and returns that element
          document.querySelector(
            ".modal-title"
          ).innerHTML = `Trip Details (Bike: ${currentTrip.bikeid})`;
          document.querySelector("#map-details").innerHTML = `Start Location: ${
            currentTrip["start station name"]
          }<br>End Location: ${currentTrip["end station name"]}<br>Duration: ${(
            currentTrip.tripduration / 60
          ).toFixed(2)}`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("shown.bs.modal", function () {
      ...
      let start = L.marker([
        currentTrip["start station location"].coordinates[1],
        currentTrip["start station location"].coordinates[0],
      ])
      ...
      let end = L.marker([
        currentTrip["end station location"].coordinates[1],
        currentTrip["end station location"].coordinates[0],
      ])
